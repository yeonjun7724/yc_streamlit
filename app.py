...
for title, df in [("AS-IS", gdf_asis), ("TO-BE", gdf_tobe)]:
    with (col1 if title=="AS-IS" else col2):
        st.markdown(f"#### {('⬅ ' if title=='AS-IS' else '')}{title}{(' ➡' if title=='TO-BE' else '')}")
        try:
            ...
            for idx, (_, row) in enumerate(c_pts.iterrows()):
                ...
                folium.Marker(location=(pt.y, pt.x), icon=icon).add_to(fg)

            # D 점
            d = d_pts.iloc[0].geometry
            d_icon = BeautifyIcon(
                icon="flag", icon_shape="marker",
            ...
            for i in range(len(coords)):
                ...
                line = LineString(res.json()["routes"][0]["geometry"]["coordinates"])
